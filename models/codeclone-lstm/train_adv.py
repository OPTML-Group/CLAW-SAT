...
def load_data(data_path, 
            fields=(SourceField(),SourceField(),SourceField(), torchtext.data.Field(sequential=False, use_vocab=False), torchtext.data.Field(sequential=False, use_vocab=False), torchtext.data.Field(sequential=False, use_vocab=False)), 
            filter_func=lambda x: True):
    src,src_adv, tgt, poison_field, idx_field,label_field = fields
    fields_inp = []
    ...
    return data, fields_inp, src,src_adv, tgt, poison_field, idx_field,label_field 
...
train, fields, src,src_adv, tgt, poison_field, idx_field,label_field = load_data(opt.train_path, filter_func=len_filter)
dev, dev_fields, src,src_adv, tgt, poison_field, idx_field,label_field = load_data(opt.dev_path, fields=(src, src_adv,tgt, poison_field, idx_field,label_field), filter_func=len_filter)


attacks = [field[0] for field in fields if field[0] not in ['src', 'tgt','index','poison','label']]
