...
dev, dev_fields, src, tgt, poison_field, idx_field,label_field = load_data(opt.dev_path, fields=(src, tgt, poison_field, idx_field,label_field), filter_func=len_filter)
...
if opt.finetune:
    ...
    tgt.build_vocab(train.src, train.tgt, max_size=params['tgt_vocab_size'],specials=replace_tokens)
...
if not opt.resume:
    ...
    if opt.finetune:
        ...
        seq2seq.encoder.load_state_dict(encoder_state_dict)
        # freeze the encoder
    if opt.fix_encoder:
    ...
    #
    optimizer = Optimizer(torch.optim.Adam(seq2seq.parameters(),lr=opt.lr),max_grad_norm=5)
