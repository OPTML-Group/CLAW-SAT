...
class EncoderRNN(BaseRNN):
    ...
    def forward(self, input_var,input_var_2, input_lengths=None,input_lengths_2=None,embedded=None, already_one_hot=False):
        """
        Applies a multi-layer RNN to an input sequence.

        Args:
            input_var (batch, seq_len): tensor containing the features of the input sequence.
            input_var2 (batch, seq_len): tensor containing the features of the input sequence.
            input_lengths (list of int, optional): A list that contains the lengths of sequences
            input_lengths_2 (list of int, optional): A list that contains the lengths of sequences
            in the mini-batch
        ...
        if embedded is None:
            if not already_one_hot:
                ...
                input_var_2 = self.convert_to_onehot(input_var_2)
                if self.half_precision:
                    input_var = input_var.half()
                    input_var_2 = input_var_2.half()
            self.input_onehot = input_var
            self.input_onehot_2 = input_var_2
            embedded = self.embedding(input_var)
            embedded = self.input_dropout(embedded) 
            embedded_2 = self.embedding(input_var_2)
            embedded_2 = self.input_dropout(embedded_2)
            '''
            ...
            if self.variable_lengths:
                ...
                embedded_2 = nn.utils.rnn.pack_padded_sequence(embedded_2, input_lengths_2, batch_first=True, enforce_sorted=False)
        else:
            embedded = Variable(embedded, requires_grad=True)
            embedded_2 = Variable(embedded_2, requires_grad=True)
            if self.variable_lengths:
                embedded = nn.utils.rnn.pack_padded_sequence(embedded, input_lengths, batch_first=True, enforce_sorted=False)
                embedded_2 = nn.utils.rnn.pack_padded_sequence(embedded_2, input_lengths_2, batch_first=True, enforce_sorted=False)
        # embedded_2 = Variable(embedded_2, requires_grad=True)
        ...
        self.embedded_2 = embedded_2
        output, hidden = self.rnn(embedded)
        output_2, hidden_2 = self.rnn(embedded_2)
        if self.variable_lengths:
            output, _ = nn.utils.rnn.pad_packed_sequence(output, batch_first=True)
            output_2, _ = nn.utils.rnn.pad_packed_sequence(output_2, batch_first=True)
        # projection for contracode pretraining
        if self.projection:
            rep = self.project(output, hidden[0])
            rep2 = self.project(output_2,hidden_2[0])
            return rep,rep2

        return output, hidden,output_2, hidden_2
