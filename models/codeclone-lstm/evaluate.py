...
def load_data(data_path, 
			fields=(SourceField(), SourceField(), SourceField(), torchtext.data.Field(sequential=False, use_vocab=False),torchtext.data.Field(sequential=False, use_vocab=False)), 
	...
	return data_all, data_sml, data_med, data_lrg, fields_inp, src, tgt, src_adv, idx_field,label_field
...
def load_model_data_evaluator(expt_dir, model_name, data_path, batch_size=128):
	...
	data_all, data_sml, data_med, data_lrg, fields_inp, src, tgt, src_adv, idx_field,label_field = load_data(data_path)
...
def evaluate_model(evaluator, model, data_all, data_sml, data_med, data_lrg, save=False, output_dir=None, output_fname=None, src_field_name='src', get_attributions=False,get_reps=False):
	print('Sizes:\n  + Data Small {}\n  + Data Med {}\n  + Data Lrg {}'.format(
	...
	print(get_reps)
	d = evaluator.evaluate_adaptive_batch(model, data_sml, data_med, data_lrg, verbose=True, src_field_name=src_field_name,get_reps=get_reps)
	...
	if save:
		if os.path.exists('/mnt/inputs/stats.json'):
			with open('/mnt/inputs/stats.json', 'r') as f:
				results = json.load(f)
		else:
			results = {} 
		
		metrics_list = ['f1', 'precision', 'recall', 'loss', 'acc']
		for m in d['metrics']:
...
if __name__=="__main__":
	...
	if opt.src_field_name == 'all':
		...
		for field_name, _ in fields_inp:
			...
			evaluate_model(evaluator, model, data_all, data_sml, data_med, data_lrg, opt.save, opt.output_dir, output_fname, field_name, opt.attributions,opt.get_reps)
	else:
		evaluate_model(evaluator, model, data_all, data_sml, data_med, data_lrg, opt.save, opt.output_dir, output_fname, opt.src_field_name, opt.attributions,opt.get_reps)
