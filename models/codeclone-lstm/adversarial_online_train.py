            ...
            for alt_iters in range(n_alt_iters):
                ...
                if u_optim and alt_iters%2 == 0:
                    ...
                    if use_u_discrete:
                        ...
                        # m = m.float()
                        # input_hot_2= input_hot_2.float()
                        decoder_outputs = model(m.half(),input_hot_2, input_lengths.cpu(),input_lengths_2.cpu(), label, already_one_hot=True)
                    else:
                        decoder_outputs = model(inputs_oho,input_hot_2, input_lengths.cpu(),input_lengths_2.cpu(), label, already_one_hot=True)
                    loss = loss_obj(decoder_outputs,label)
                    if torch.isnan(loss):
...
if __name__=="__main__":
    ...
    if opt.resume:
        ...
        start_epoch = 1
        step = 0
        optimizer = Optimizer(torch.optim.SGD(model.parameters(),lr=1e-4), max_grad_norm=5)
