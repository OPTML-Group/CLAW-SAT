...
def load_data(data_path, 
    ...
    src, tgt, src_adv, idx_field,label_field = fields
    ...
    with open(data_path, 'r') as f:
        ...
        for col in cols:
            if col=='src':
                ...
                fields_inp.append(('label', idx_field))
            else:
    ...
    return data, fields_inp, src, tgt, src_adv, idx_field,label_field
...
def apply_gradient_attack_v2(data, model, input_vocab, replace_tokens, field_name, opt, orig_tok_map, idx_to_fname, output_vocab=None, device='cpu'):
    ...
    for params in itertools.product(li_u_optim_technique, li_u_init_pgd, li_learning_rate, li_use_loss_smoothing, li_use_u_discrete):
        ...
        batch_obj = torch.nn.CrossEntropyLoss(reduction="none")
        model.train()
        ...
        for bid, batch in enumerate(tqdm.tqdm(batch_generator, total=len(batch_iterator))):
            ...
            # Do random attack if inputs are too long and will OOM under gradient attack
            if u_optim and max(getattr(batch, field_name)[1]) > 1000:
            ...
            if (u_optim or z_optim) and use_orig_tokens:
                ...
                input_hot_2 = Variable(convert_to_onehot(input_variables_2, vocab_size=len(input_vocab), device=device), requires_grad=True).half()     
                ...
                if u_optim and alt_iters%2 == 0:
                    ...
                    for j in range(u_pgd_epochs):
                        ...
                        if use_u_discrete:
                            ...
                            decoder_outputs = model(m,input_hot_2, input_lengths.cpu(),input_lengths_2.cpu(), label, already_one_hot=True)
                        else:
                            decoder_outputs = model(inputs_oho,input_hot_2, input_lengths.cpu(),input_lengths_2.cpu(), label, already_one_hot=True)
                        # loss = loss_obj(decoder_outputs,label)
                        token_wise_loss_per_batch = batch_obj(decoder_outputs,label)
...
if __name__=="__main__":
    ...
    data, fields_inp, src, tgt, src_adv, idx_field,label_field = load_data(opt.data_path)
    ...
    if not opt.no_gradient:
        ...
        for field_name, _ in fields_inp:
            if field_name in ['src', 'tgt', 'index', 'transforms.Identity','label']:
                continue
