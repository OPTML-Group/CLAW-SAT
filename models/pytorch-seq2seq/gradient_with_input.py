...
def apply_gradient_attack_v2(data, model, input_vocab, replace_tokens, field_name, opt, orig_tok_map, idx_to_fname, output_vocab=None, device='cpu'):
    ...
    li_u_init_pgd = [1] #list(range(5)) # 0: Original (fixed) init; 1: randomly initalize all tokens; 2: pick PGD optimal randomly instead of argmax; >2: randomly initialize only z=true; 
    li_learning_rate = [1]
    ...
    for params in itertools.product(li_u_optim_technique, li_u_init_pgd, li_learning_rate, li_use_loss_smoothing, li_use_u_discrete):
        ...
        for bid, batch in enumerate(tqdm.tqdm(batch_generator, total=len(batch_iterator))):
            ...
            input_variables, input_lengths = getattr(batch, field_name)
            target_variables = getattr(batch, 'tgt')
            orig_input_variables, input_lengths = getattr(batch, 'src')
            ...
            if (u_optim or z_optim) and use_orig_tokens:
                ...
                inputs_oho = Variable(convert_to_onehot(torch.tensor(orig_input_variables, device=device), vocab_size=len(input_vocab), device=device), requires_grad=True).half()    
                    ...
                    for j in range(u_pgd_epochs):
                        ...
                        if use_loss_smoothing:
                            ...
                            for si in range(smooth_iters):
                                ...
                                smooth_decoder_outputs, smooth_decoder_hidden, smooth_other = model(smooth_input, input_lengths.cpu(), target_variables, already_one_hot=True)
                                if use_cw_loss:
                        ...
                        for i in range(input_variables.shape[0]):
                            ...
                            gradients_np = np.zeros(gradients.shape)
                            
                            input_int = np.argmax(input_hot,axis=-1)
                            print(input_int.shape)
                            print(gradients_np.shape)
                            for i in range(input_int.size(1)):
                                gradients_np[:,i] = gradients[input_int[0,i]]
                            print(gradients_np)
                            # print(np.sum())
                            print(index)
                            src_seq = [input_vocab.itos[int(tok)] for tok in input_int]
                            # tgt_seq = [output_vocab.vocab.itos[tok] for tok in tgt_id_seq]
                            gt = [output_vocab.vocab.itos[tok] for tok in target_variables[i]]
                            print("src:",src_seq)
                            print("gt:",gt)
    ...
    best_replacements_dataset_all = 0
    stats = 0
    best_replacements_dataset=0
