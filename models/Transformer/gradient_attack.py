...
def parse_args():
    ...
    parser.add_argument('--teacher_ratio', action='store_true', default=False)
...
def apply_gradient_attack(data, model, input_vocab, replace_tokens, field_name, opt, output_vocab=None):
    ...
    for batch in tqdm.tqdm(batch_generator, total=len(batch_iterator)):
        ...
        input_onehot = convert_to_onehot(input_variables, vocab_size=len(input_vocab), device=device).half()
        # Forward propagation       
...
def apply_gradient_attack_v2(data, model, input_vocab, replace_tokens, field_name, opt, orig_tok_map, idx_to_fname, output_vocab=None, device='cpu'):
    ...
    li_u_optim_technique = [2] # 1: PGD: SGD with relaxation; 2: signed gradient
    li_u_init_pgd = [2] #list(range(5)) # 0: Original (fixed) init; 1: randomly initalize all tokens; 2: pick PGD optimal randomly instead of argmax; >2: randomly initialize only z=true; 
    li_learning_rate = [opt.u_learning_rate]
    ...
    for params in itertools.product(li_u_optim_technique, li_u_init_pgd, li_learning_rate, li_use_loss_smoothing, li_use_u_discrete):
        ...
        for bid, batch in enumerate(tqdm.tqdm(batch_generator, total=len(batch_iterator))):
            ...
            indices = getattr(batch, 'index')
            input_variables, input_lengths = getattr(batch, field_name)
            # print("input size:",input_variables.shape)
            # print(input_variables)
            
            target_variables = getattr(batch, 'tgt')
            input_target = target_variables[:,:-1]
            # print(input_target)
            output_target = target_variables[:,1:]
            # orig_input_variables, orig_lens = getattr(batch, 'src')
            tot_samples += len(getattr(batch, field_name)[1])
            # Do random attack if inputs are too long and will OOM under gradient attack
            if u_optim and max(getattr(batch, field_name)[1]) > 500:
                #print(max(getattr(batch, field_name)[1]))
                rand_replacement_too_long += len(getattr(batch, field_name)[1])
                #print(rand_replacement_too_long)
            ...
            # orig_inputs_oho = convert_to_onehot(input_variables, vocab_size=len(input_vocab), device=device).half()
            # print(inputs_oho.shape)
            # print(inputs_oho)
            ...
            if (u_optim or z_optim) and use_orig_tokens:
                new_inputs, site_map_map, z_all_map, input_lengths, sites_to_fix_map = replace_toks_batch(input_variables.cpu().numpy(), indices, z_np_map, site_map_map, site_map_lookup_map, best_replacements_batch, field_name, input_vocab, orig_tok_map, idx_to_fname)
            ...
            for alt_iters in range(n_alt_iters):
                ...
                if u_optim and alt_iters%2 == 0:
                    ...
                    for i in range(input_variables.shape[0]):
                        ...
                        # print(inputs_oho.is_leaf)
                    # print("begin of gpd:",bid,inputs_oho.shape)    
                    for j in range(u_pgd_epochs):
                        ...
                        if use_u_discrete:
                            ...
                            m = torch.zeros(inputs_oho.shape, requires_grad=False, device=device).scatter(2, a.unsqueeze(2), 1.0)
                            if len(input_target.shape)>2:
                                input_target = input_target.argmax(2)
                            if opt.teacher_ratio:
                                # print("teacher_ratio")
                                decoder_outputs,seqs = model(inputs_oho, input_lengths, input_target, already_one_hot_en=True,already_one_hot_de=True,teacher_forcing_ratio=0,half_precision=True,start_token =output_vocab.vocab.stoi['<sos>'] )
                                decode_length = target_variables.shape[1]
                                new_target_variabels = torch.zeros([decoder_outputs.shape[0],decoder_outputs.shape[1]],dtype=torch.long).to(device)
                                # decoder_outputs = decoder_outputs[:,:decode_length,:]
                                torch.full([decoder_outputs.shape[0],decoder_outputs.shape[1]],fill_value=pad,device=device,out=new_target_variabels,dtype=torch.long)
                                new_target_variabels[:,:decode_length] = target_variables[:]
                                # print(new_target_variabels.dtype)
                                # print(new_target_variabels)
                                new_target_variabels=new_target_variabels.to(device)
                                # decoder_outputs = decoder_outputs[:,:decode_length,:]                            
                            else:
                                decoder_outputs = model(m, input_lengths, input_target, already_one_hot_en=True,half_precision=True,teacher_forcing_ratio=1)
                            del a,m
                        else:
                            decoder_outputs, decoder_hidden, other = model(inputs_oho, input_lengths, input_target, already_one_hot_en=True,half_precision=True)
                        #loss= F.cross_entropy(decoder_outputs.transpose(1, 2), target_variables[:, 1:],ignore_index=output_vocab.vocab.stoi[output_vocab.pad_token])
                        if opt.teacher_ratio:
                            token_wise_loss_per_batch=torch.mean(F.cross_entropy(decoder_outputs.transpose(1, 2).contiguous(), new_target_variabels.contiguous(),ignore_index=output_vocab.vocab.stoi[output_vocab.pad_token],reduction="none"),axis=1)
                        else:
                            token_wise_loss_per_batch=torch.mean(F.cross_entropy(decoder_outputs.transpose(1, 2).contiguous(), output_target.contiguous(),ignore_index=output_vocab.vocab.stoi[output_vocab.pad_token],reduction="none"),axis=1)
                        
                        token_wise_loss_per_batch=abs(token_wise_loss_per_batch.detach().cpu().numpy())
                        #print(token_wise_loss_per_batch)
                        del decoder_outputs
                            # print(token_wise_loss_per_batch)
                            # print(decoder_outputs.transpose(1, 2).shape)
                            # print(token_wise_loss_per_batch.shape)
                            # exit(0)
                        ...
                        # target_variables.requires_grad_()
                        # input_targets=Variable(convert_to_onehot(target_variables[:,:-1], vocab_size=len(output_vocab.vocab), device=device), requires_grad=True)
                        input_target=Variable(convert_to_onehot(input_target, vocab_size=len(output_vocab.vocab), device=device), requires_grad=True).half()
                        input_target = input_target + 1-1
                        # print(input_targets.is_leaf)
                        # input_targets_new = input_targets+1-1 
                        # print(input_targets_new.is_leaf)
                        # print(inputs_oho.is_leaf)
                        # print(input_target.is_leaf)
                        # inputs_oho = copy.deepcopy(inputs_oho)
                        # inputs_oho = inputs_oho.requires_grad_()
                        ...
                        if opt.teacher_ratio:
                            ...
                            loss= F.cross_entropy(decoder_outputs.transpose(1, 2).contiguous(), new_target_variabels.contiguous(),ignore_index=output_vocab.vocab.stoi[output_vocab.pad_token])
                        else:
                            decoder_outputs = model(inputs_oho, input_lengths, input_target, already_one_hot_en=True,already_one_hot_de=True,teacher_forcing_ratio=1,half_precision=True)
                            loss= F.cross_entropy(decoder_outputs.transpose(1, 2).contiguous(), output_target.contiguous(),ignore_index=output_vocab.vocab.stoi[output_vocab.pad_token])
                        # print(decoder_outputs.shape)
                        # print(seqs)
    # print(input_lengths)
                        # token_wise_loss_per_batch=F.cross_entropy(decoder_outputs.transpose(1, 2), target_variables[:, 1:],ignore_index=output_vocab.vocab.stoi[output_vocab.pad_token],reduce=False)
                        # token_wise_loss_per_batch=torch.sum(token_wise_loss_per_batch,axis=1)
                        # update loss and backprop
                        # grads_oh = torch.autograd.grad(loss,inputs_oho,create_graph=True)[0].detach().cpu().numpy()
                        model.zero_grad()
                        inputs_oho.retain_grad()
                        loss.backward(retain_graph=True)
                        grads_oh = inputs_oho.grad
                        # del loss,decoder_outputs
                        for i in range(input_variables.shape[0]):
                            ...
                            inputs_oho[i] = torch.tensor(input_hot, requires_grad=True)
                            # Done optimizing
                        ...
                        # print("after pgd one iterations:",bid,inputs_oho.shape)
                ...
                if u_optim and alt_iters%2 == 0:
                    for i in range(inputs_oho.shape[0]):
                        ...
                        fname = str(indices[i])
                        # print(best_loss_among_iters)
                        # for id,best_loss_among_iter in enumerate(best_loss_among_iters[fname]):
                        #     print(id,best_loss_among_iter)
            ...
            #             print(bid,max(getattr(batch, field_name)[1]),max(getattr(batch, "tgt")[1]),type(obj), obj.size())
            #     except:
            #         pass        
    ...
    best_replacements_dataset_all, avg_replaced = get_all_replacements(best_replacements_dataset, field_name, orig_tok_map, idx_to_fname, True)
    print('\n# tokens optimized on an average: {}'.format(avg_replaced))
    stats['n_tokens_optimized_avg'] = avg_replaced
    print("\n# of samples attacked post processing: {}\n=======".format(len(best_replacements_dataset_all.keys())))
    stats['n_samples_attacked_post_processing'] = len(best_replacements_dataset_all.keys())



    if analyze_exact_match_sample:
        for kz in kzs:
            print("{}::{}".format(kz, best_replacements_dataset_all[kz]))

    return best_replacements_dataset_all, stats, best_replacements_dataset
...
if __name__=="__main__":
    ...
    if not opt.no_gradient:
        ...
        for field_name, _ in fields_inp:
            ...
            t_start = time.time()
            d[field_name], stats, optim_tokens = attack_fname(data, model, input_vocab, replace_tokens, field_name, opt, orig_tok_map, idx_to_fname, tgt, device)
