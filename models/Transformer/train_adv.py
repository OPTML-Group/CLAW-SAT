...
import sys
import csv
# from contracode_adversarial.models.Transformer.seq2seq.models import Transformer
import numpy as np
...
def load_data(data_path, 
            fields=(TransSourceField(), TransSourceField(), TargetField(),Field(sequential=False, use_vocab=False),Field(sequential=False, use_vocab=False)), 
...
if opt.resume:
    if opt.load_checkpoint is None:
        ...
        if opt.fix_encoder:
            ...
            transformer.module.encoder.eval()
else:
...
# train 
t = SupervisedAdversarialTrainer(loss="nll_tokens", batch_size=params['batch_size'],
                      checkpoint_every=50,
                      print_every=100, expt_dir=opt.expt_dir, tensorboard=True, 
                      batch_adv_loss=batch_adv_loss,transformer=True,ignore_index=tgt.vocab.stoi['<pad>'],dev_path=opt.dev_path,input_vocab=src.vocab,output_vocab=tgt.vocab)
