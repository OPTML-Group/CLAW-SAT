...
def attack_and_analyze_model(model, data, attacks, src_vocab, tgt_vocab):
    ...
    for i in outputs:
        ...
        with open(os.path.join(opt.output_dir,'attacked_metrics_Q_%s.txt'%i), 'w') as f:
            f.write(json.dumps(metrics)+'\n')

        print('depth:',i)
        print(metrics)

        print('Metrics written to', os.path.join(opt.output_dir,'attacked_metrics_Q_%s.txt'%i))
        print()
