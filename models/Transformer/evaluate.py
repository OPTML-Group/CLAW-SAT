...
def parse_args():
	...
	parser.add_argument('--get_reps',action='store',default=False)
...
def load_data(data_path, 
			fields=(TransSourceField(), TargetField(), TransSourceField(), torchtext.data.Field(sequential=False, use_vocab=False)), 
	...
	def len_filter_all(example):
		return len_filter_sml(example) or len_filter_med(example)
	...
	data_all = torchtext.data.TabularDataset(
		...
		skip_header=True, 
		csv_reader_params={'quoting': csv.QUOTE_NONE}, 
		filter_pred=len_filter_all
	)
	return data_all, data_sml, data_med, data_lrg, fields_inp, src, tgt, src_adv, idx_field


def load_model_data_evaluator(expt_dir, model_name, data_path,batchsize):
	...
	return model, data_all, data_sml, data_med, data_lrg, fields_inp
...
def evaluate_model( model, data_all, data_sml, data_med, data_lrg, save=False, output_dir=None, output_fname=None, src_field_name='src', get_attributions=False,get_reps=False,batch_size=128):
	print('Sizes:\n  + Data Small {}\n  + Data Med {}\n  + Data Lrg {}'.format(
	...
	print(get_reps)
	d = _evaluate(model,data_all,loss_type="nll_tokens",src_field_name=src_field_name,batch_size=batch_size,decode_type="greedy")
	...
	if get_reps:
		print('---------------', end=' ')
		print('%s: %s\n'%('mean_distances',d['mean_distances']))		
	if save:
		...
		with open(os.path.join(output_dir,'results.json'), 'w') as f:
			json.dump(results, f)
		if get_reps:
			with open(os.path.join(output_dir,'dis.json'), 'w') as f:
				json.dump(d['distances'], f)			
...
if __name__=="__main__":
	...
	model, data_all, data_sml, data_med, data_lrg, fields_inp = load_model_data_evaluator(opt.expt_dir, model_name, opt.data_path, opt.batch_size)
	# device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
	# batch_iterator = torchtext.data.BucketIterator(
	# 	dataset=data_all, batch_size=opt.batch_size,
	# 	sort=False, sort_within_batch=True,
	# 	sort_key=lambda x: len(getattr(x,opt.src_field_name)),
	# 	device=device, repeat=False)
	
	# batch_generator = batch_iterator.__iter__()
	# max_length_input = 0 
	# for batch in batch_generator:
	# 	X, X_lengths = getattr(batch, opt.src_field_name)
	# 	max_len = torch.max(X_lengths)
	# 	if max_len > max_length_input:
	# 		max_length_input=max_len
	...
	if opt.src_field_name == 'all':
		...
		for field_name, _ in fields_inp:
			...
			evaluate_model( model, data_all, data_sml, data_med, data_lrg, opt.save, opt.output_dir, output_fname, field_name, opt.attributions,opt.get_reps,opt.batch_size)
	else:
		print(opt.src_field_name)
