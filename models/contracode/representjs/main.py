...
from data.util import Timer, get_seq2seq_data_generator
...
def plot_loss(train, val, plot_name):
    plt.plot(train)
    plt.plot(val)
    plt.ylabel('loss')
    plt.xlabel('epoch')
...
def _evaluate(
    model, loader, sp: spm.SentencePieceProcessor, use_cuda=True, num_to_print=8, beam_search_k=5, max_decode_len=20, loss_type="nll_token", model_type=None, vocab=None
):
    model.eval()
    ...
    with torch.no_grad():
        ...
        with Timer() as t:
            ...
            total_loss = 0
            num_examples = 0
            pbar = tqdm.tqdm(loader, desc="evaluate")
            for batch in pbar:
                if model_type == "seq2seq":
                ...
                if model_type == "seq2seq":
                    # X_lengths = X_lengths.cpu().numpy().tolist()
                    # Y = Y[:, :-1]
                    logits_list = model(X, X_lengths, Y)[2]['logits']
                    logits = torch.stack(logits_list, dim=1) # B x seq_len x vocab_size
                else:
    ...
    with Timer() as t:
        ...
        with tqdm.tqdm(test_loader, desc="test") as pbar:
            for batch in pbar:
                ...
                with Timer() as t:
                    if model_type == "seq2seq":
                        ret_dict = model(X, X_lengths)[2]
                        sequence = ret_dict["sequence"]
                        pred_ids = torch.squeeze(torch.stack(sequence, dim=1))
                        pred = ids_to_strs(pred_ids, sp)
...
def calculate_nll(model, test_loader, sp: spm.SentencePieceProcessor, use_cuda=True, logger_fn=None, model_type=None):
    pad_id = sp.PieceToId("[PAD]")
    n_examples = 0
    test_nll = 0.0
    with tqdm.tqdm(test_loader, desc="Test (NLL)") as pbar:
        for batch in pbar:
            ...
            if model_type == "seq2seq":
                ...
                # Y = Y[:, :-1]
                logits_list = model(X, X_lengths, Y)[2]['logits']
                pred_y = torch.stack(logits_list, dim=1) # B x seq_len x vocab_size
            else:
...
def test(
    ...
    if use_cuda:
        ...
        device = "cuda"
    sp, input_vocab, output_vocab = None, None, None
    if model_type == "seq2seq":
        with open(os.path.join(vocab_filepath, 'input_vocab.pt'), 'rb') as f:
            input_vocab = dill.load(f)
        with open(os.path.join(vocab_filepath, 'output_vocab.pt'), 'rb') as f:
            output_vocab = dill.load(f)
        pad_id = input_vocab.stoi["<pad>"]
    else:
        sp = spm.SentencePieceProcessor()
    ...
    if model_type == "seq2seq":
        test_loader, _ = get_seq2seq_data_generator(test_filepath, input_vocab, output_vocab, batch_size, device, max_length)
    else:
    ...
    if model_type == "transformer":
        ...
        encoder = EncoderRNN(len(input_vocab), max_length, d_model,
                            bidirectional=True, variable_lengths=True, 
                            n_layers=2, rnn_cell='lstm', half_precision=True)
        decoder = DecoderRNN(len(output_vocab), max_length, d_model * 2,
                            dropout_p=0.2, use_attention=True, bidirectional=True, 
                            rnn_cell='lstm', n_layers=2, 
                            eos_id=output_vocab.stoi["<eos>"], sos_id=output_vocab.stoi["<sos>"])
        model = Seq2seq(encoder, decoder).half()
        logger.info(f"Created Seq2Seq with {count_parameters(model)} params")
    ...
    with torch.no_grad():
        precision, recall, f1, sample_generations = calculate_f1_metric(
            ...
            sp,
            use_cuda=use_cuda,
            use_beam_search=(not model_type == "seq2seq"),
    ...
    logger.info(f"Precision: {precision*100:.5f}%")
    ...
    model.eval()
    with torch.no_grad():
        test_nll = calculate_nll(model, test_loader, sp, use_cuda=use_cuda, model_type="seq2seq")
...
def train(
    ...
    if model_type == "seq2seq":
        train_loader, train_loader_len = get_seq2seq_data_generator(train_filepath, input_vocab, output_vocab, batch_size, device, max_length)
        train_loader_len *= batch_size
    else:
    ...
    if model_type == "seq2seq":
        eval_loader, _ = get_seq2seq_data_generator(eval_filepath, input_vocab, output_vocab, batch_size, device, max_length)
    else:
    ...
    if use_lr_warmup:
        scheduler = get_linear_schedule_with_warmup(optimizer, 5000, train_loader_len * num_epochs)
    else:
        ...
        if resume_encoder_name == "supervised":
            ...
            for key, value in pretrained_state_dict.items():
                ...
                if key.startswith(resume_encoder_name + ".") and "project_layer.0." in key and resume_project:
                    ...
                    encoder_state_dict[remapped_key] = value
            model.module.encoder.load_state_dict(encoder_state_dict, strict=False)
    ...
    for epoch in tqdm.trange(start_epoch, num_epochs + 1, desc="training", unit="epoch", leave=False):
        ...
        for batch in pbar:
            ...
            num_examples += X.size(0)
        train_losses.append(train_loss_total/num_examples)
        # Evaluate
        logger.info(f"Evaluating model after epoch {epoch} ({global_step} steps)...")
        max_decode_len = 20 if label_mode == "identifier" else 200
        eval_loss = _evaluate(model, eval_loader, sp, use_cuda=use_cuda, max_decode_len=max_decode_len, loss_type=loss_type, model_type=model_type, vocab=input_vocab)
        ...
        plot_file = run_dir / "loss.png"
        plot_loss(train_losses, eval_losses, plot_file)
