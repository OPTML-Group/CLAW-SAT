...
if __name__ == "__main__":
  ...
  for split in ["test","train","valid"]:
    ...
    with open('mnt/train_file/{}.txt'.format(split),'r') as f:
      ...
      while(line):
        ...
        if idx1 in index_number_map and idx2 in index_number_map:
          index_lst.append(index_number_map[idx1])
          cnt+=1
        line = f.readline()
    ...
    # exit(0)
      tasks.append((split,idx1,code_info[idx1],code_info[idx1]))
      if get_site_map:
        new_site_map[idx1] = {}
        # if from_file in site_map:
        for r in site_map[idx1]:
          if site_map[idx1][r][0] == '':
            new_site_map[idx1][r] = site_map[idx1][r]
          else:
            new_site_map[idx1][r] = (' '.join([normalize_subtoken(subtok) for subtok in subtokens([site_map[idx1][r][0]])]), site_map[idx1][r][1])
  ...
  for good, split,idx ,src, tgt in iterator:
    ...
    from_file_name = (idx).replace('\0', ' ')
    src = src.replace('\0', ' ')
    tgt = tgt.replace('\0',' ')
    # label = label.replace('\0',' ')
    out_map[split].write(
      '{}\t{}\t{}\n'.format(from_file_name, src, tgt)
    )
