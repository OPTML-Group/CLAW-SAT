...
export FRE=$FRE
echo "$FRE"
 echo "[ADV-TRAIN] Copying over ${MODEL_NAME} model for first-round targeting..."
 mkdir -p "final-models/seq2seq/code_completion/${SHORT_NAME}/${DATASET}/${TRANSFORMS}/adversarial/lstm"
 docker run -it --rm \
   -v "${DIR}/../final-models/seq2seq/code_completion/${DATASET}/${MODEL_NAME}":/mnt/inputs \
   -v "${DIR}/../final-models/seq2seq/code_completion/${SHORT_NAME}/${DATASET}/${TRANSFORMS}/adversarial":/mnt/outputs \
   debian:9 \
...
STATS_FILE="final-models/seq2seq/code_completion/${SHORT_NAME}/${DATASET}/${TRANSFORMS}/adversarial/stats.txt"
...
MODELS_IN=final-models/seq2seq/code_completion/${SHORT_NAME}/${DATASET}/${TRANSFORMS}/adversarial \
...
PLOT_DIR=plots/${SHORT_NAME}
if [ ! -d "${PLOT_DIR}" ]; then
    mkdir ${PLOT_DIR}
fi

ARGS="${DO_FINETUNE} --epochs 1 --batch_size 16 --lamb ${LAMB}" \
SHORT_NAME="${SHORT_NAME}" \
GPU="${GPU}" \
DECODER_ONLY="${DECODER_ONLY}" \
MODELS_OUT=final-models/seq2seq/code_completion/${SHORT_NAME}/${DATASET}/${TRANSFORMS} \
DATASET_NAME=datasets/code_completion/adversarial/${SHORT_NAME}/tokens/${DATASET}/${TARGETING}-targeting \
TRAIN_INFO_DIR=plots/${SHORT_NAME} \
  time make adv-train-model-seq2seq-code-completion
...
export c=`expr $epoch % $FRE`
if [ "$c" == "0" ]; then
echo "need to remove dataset"
